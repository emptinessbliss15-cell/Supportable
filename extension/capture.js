...
async function start() {
  try {
    ...
    const screenVideo = document.createElement("video"); screenVideo.srcObject = screenStream; screenVideo.muted = true; await screenVideo.play();
    ...
    if (micStream) { audioContext = new AudioContext(); const destination = audioContext.createMediaStreamDestination(); audioContext.createMediaStreamSource(micStream).connect(destination); destination.stream.getAudioTracks().forEach(track => output.addTrack(track)); }
    preview.srcObject = cameraStream || screenStream;
    const mimeTypes = ["video/webm;codecs=vp9,opus", "video/webm;codecs=vp8,opus", "video/webm"];
    const mimeType = mimeTypes.find(type => MediaRecorder.isTypeSupported(type));
    recorder = new MediaRecorder(output, mimeType ? { mimeType } : undefined); chunks = [];
    ...
    recorder.start(1000); startedAt = Date.now(); timerHandle = setInterval(() => { timer.textContent = formatTime(Date.now() - startedAt); }, 250);
    startButton.disabled = true; stopButton.disabled = false; status.textContent = "Recording";
    message.textContent = cameraStream ? "Screen + camera overlay" : "Screen recording";
  } catch (error) { stopTracks(); status.textContent = "Ready"; message.textContent = error instanceof Error ? error.message : "Capture was cancelled."; }
}
...
async function saveRecording() {
  stopTracks(); const blob = new Blob(chunks, { type: recorder.mimeType || "video/webm" }); const id = crypto.randomUUID();
  ...
  const db = await openDb(); await putAttachment(db, { ...record, blob }); await chrome.storage.local.set({ latestCapture: record });
  status.textContent = "Recording saved"; message.textContent = `Saved ${record.name} (${Math.round(record.size / 1024 / 1024 * 10) / 10} MB). Return to the extension.`; startButton.disabled = false; preview.srcObject = null;
}
